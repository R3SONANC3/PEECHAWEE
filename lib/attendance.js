...
// Unlocked core — callers that already hold the 'attendance' write lock
// (saveAttendance) call this directly instead of the exported, locked
// getAttendanceForDate, to avoid deadlocking on their own lock.
async function readAttendance(dateLabel) {
...
// Locked: syncNamesFromRoster can itself clear+rewrite the whole sheet when
// the roster changed, so even this "read" can race a concurrent save.
export async function getAttendanceForDate(dateLabel) {
  return withWriteLock('attendance', () => readAttendance(dateLabel));
}

export async function saveAttendance(dateLabel, records) {
  return withWriteLock('attendance', () => doSaveAttendance(dateLabel, records));
}

async function doSaveAttendance(dateLabel, records) {
  ...
  return readAttendance(dateLabel);
}
