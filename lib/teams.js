...
// Keep in sync with WAR_SHEETS + LEGACY_ATTENDANCE_SHEET_NAME in
// lib/attendance.js — not imported directly to avoid a circular dependency
// (attendance.js depends on this module, for its own team-sheet name sync).
const NON_TEAM_SHEET_NAMES = ['เช็คชื่อ MAIN WAR', 'เช็คชื่อ SUB WAR', 'เช็คชื่อกิลวอร์'];
...
// Any tab that isn't the roster (first tab), one of the attendance sheets,
// or the shared GEAR sheet counts as a team sheet — so adding a new
// War/Polarity-style tab to the spreadsheet makes it show up here
...
export async function listTeamSheetTitles() {
  ...
  return sorted
    .filter((t) => t.title !== rosterTitle && !NON_TEAM_SHEET_NAMES.includes(t.title) && t.title !== GEAR_SHEET_NAME)
    .map((t) => t.title);
}
...
// next team header in the same column, capped at MAX_TEAM_SIZE. The cap
// matters because sections (TOP/MID/BOT-style groups) reuse the same columns
// stacked vertically with blank separator rows in between — without the cap,
// a team's block would be read as reaching all the way down to the next
// section's header, swallowing those blank rows.
//
// When there's no next header below in the same column (the last team in
// that column), don't fall back to gridLength: the Sheets API trims trailing
// rows that are blank across the WHOLE sheet, so a team whose slots 2-5
// haven't been filled in yet gets a short, wrong grid length — reading back
// as if it only had 1 slot. Every team has 5 slots by guild rule regardless
// of how many are filled in, so assume the full cap instead.
function blockSlotCount(headers, header, gridLength) {
  let blockEnd = null;
  headers.forEach((other) => {
    if (other.c === header.c && other.r > header.r && (blockEnd === null || other.r < blockEnd)) blockEnd = other.r;
  });
  if (blockEnd === null) return MAX_TEAM_SIZE;
...
// One lock per duplicate-name group (Main-War+SUB-WAR share one; Polarity
// and Castle each get their own) so a slot write and its own conflict check
// can't interleave with another write to the same group — without this, two
// near-simultaneous assigns of the same name to two different slots could
// both read "free" before either write lands.
function lockKeyFor(sheetTitle) {
  return `teams:${duplicateGroupSheets(sheetTitle).join('+')}`;
}

function conflictError(name, existing) {
  const err = new Error(`"${name}" ถูกจัดอยู่แล้วที่ ${existing.sheet} · ${existing.team} · ช่องที่ ${existing.slot}`);
...
export async function updateTeamSlot(sheetTitle, teamKey, slot, name) {
  return withWriteLock(lockKeyFor(sheetTitle), () => doUpdateTeamSlot(sheetTitle, teamKey, slot, name));
}

async function doUpdateTeamSlot(sheetTitle, teamKey, slot, name) {
  const grid = await getValues(quoteSheet(sheetTitle), 'UNFORMATTED_VALUE');
  const headers = findTeamHeaders(grid);
  ...
  if (trimmedName) {
    const assignments = await readGroupAssignments(sheetTitle);
    const existing = assignments[trimmedName];
    const isSameSlot = existing && existing.sheet === sheetTitle && existing.teamKey === teamKey && existing.slot === slotNum;
    if (existing && !isSameSlot) throw conflictError(trimmedName, existing);
  }
...
// Blanks every slot in one team in a single batched write, so an admin can
// wipe a team clean before re-typing a fresh lineup instead of clicking
// delete on each slot individually.
export async function clearTeam(sheetTitle, teamKey) {
  return withWriteLock(lockKeyFor(sheetTitle), () => doClearTeam(sheetTitle, teamKey));
}

async function doClearTeam(sheetTitle, teamKey) {
  const grid = await getValues(quoteSheet(sheetTitle), 'UNFORMATTED_VALUE');
  const headers = findTeamHeaders(grid);
  const header = headers.find((h) => `${h.r}:${h.c}` === teamKey);
  if (!header) throw new Error('team not found');
  ...
  const nameCol = colLetter(header.c + 1);
  const rows = Array.from({ length: slotCount }, () => ['']);
  await setValues(`${quoteSheet(sheetTitle)}!${nameCol}${header.r + 2}:${nameCol}${header.r + 1 + slotCount}`, rows);
...
export async function importTeam(targetSheet, targetTeamKey, sourceSheet, sourceTeamKey) {
  return withWriteLock(lockKeyFor(targetSheet), () => doImportTeam(targetSheet, targetTeamKey, sourceSheet, sourceTeamKey));
}

async function doImportTeam(targetSheet, targetTeamKey, sourceSheet, sourceTeamKey) {
...
// A player removed from the roster (deleted, or renamed away from their old
// name) shouldn't linger on a team slot under a name that no longer exists
// there — called from the roster API on delete/rename. Checks every team
// sheet (Main-War, SUB-WAR, Castle, Polarity, and any future one) since a
// name can hold a slot in more than one independent duplicate-check group
// at once (e.g. a Main-War team AND a Castle team).
export async function removeNameFromAllTeams(name) {
  ...
  for (const title of sheetTitles) {
    ...
    for (const team of teams) {
      const member = team.members.find((m) => m.name === trimmed);
      if (member) await updateTeamSlot(title, team.key, member.slot, '');
    }
  }
}
