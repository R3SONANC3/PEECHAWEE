...
จัดการรายชื่อสมาชิก เช็คชื่อกิลวอร์ และจัดทีม War/Polarity — เขียนด้วย Next.js (JavaScript ธรรมดา ไม่ใช้ TypeScript) โดยใช้ Google Sheet เดิมเป็นฐานข้อมูล ผ่าน Google Sheets API ตรง ๆ (ไม่ใช้ Google Apps Script อีกต่อไป)
...
- **ชีต `Main-War` / `Polarity`**: หัวทีมเป็นเซลล์ข้อความ `TEAM 1`, `TEAM 2`, ... ตามด้วย 5 แถวถัดไปในคอลัมน์เดียวกัน = ชื่อสมาชิก และ **คอลัมน์ถัดไปทางขวา = ค่า GEAR** ของคนนั้น
...
- `app/*/page.js` — แต่ละหน้า (roster, attendance, war, polarity) โหลดข้อมูลฝั่ง server ตั้งแต่แรกเพื่อความไว
