...
ADMIN_PASSWORD=<ตั้งรหัสผ่านผู้บริหารเอง>
```

`ADMIN_PASSWORD` คือรหัสผ่านที่ใช้เข้าโหมด "ผู้บริหาร" (แก้ไขข้อมูลได้) — แจกให้เฉพาะทีมผู้บริหาร ใครไม่มีรหัสจะเห็นเว็บแบบดูอย่างเดียว **ถ้าไม่ตั้งค่านี้ จะไม่มีใครเข้าโหมดผู้บริหารได้เลย ไม่ว่าจะพิมพ์รหัสอะไรก็ตาม**
...
- **ชีต GEAR**: แท็บชื่อ `Gear` (คอลัมน์ A = ชื่อ, B = ค่า GEAR) — เก็บค่า GEAR ของแต่ละคนไว้ที่เดียว ให้ทุกหน้าทีมดึงไปโชว์ตรงกัน แทนที่จะพิมพ์แยกไว้ในแต่ละชีตทีมเหมือนเดิม **แท็บนี้ถูกสร้างอัตโนมัติในครั้งแรกที่มีใครเปิดเว็บหลัง deploy** — พร้อมย้ายค่า GEAR ที่เจอล่าสุดของแต่ละคนจากชีตทีมต่าง ๆ มาใส่ให้ ไม่ต้องสร้างแท็บเองล่วงหน้า

## Deploy ขึ้น Vercel

1. Push โค้ดนี้ขึ้น GitHub (ทำให้แล้วในโปรเจกต์นี้)
2. เข้า [vercel.com](https://vercel.com) → **Add New > Project** → เลือก repo `PEECHAWEE`
3. ก่อนกด Deploy ให้ไปที่ **Environment Variables** เพิ่ม 4 ตัวเดียวกับใน `.env.local`:
   - `GOOGLE_SHEET_ID`
   - `GOOGLE_SERVICE_ACCOUNT_EMAIL`
   - `GOOGLE_PRIVATE_KEY` (ใส่ทั้งก้อนพร้อม `\n` เหมือนใน `.env.local`)
   - `ADMIN_PASSWORD` (ลืมตั้งตัวนี้ = ไม่มีใครเข้าโหมดผู้บริหารได้)
...
- `lib/roster.js`, `lib/attendance.js`, `lib/teams.js`, `lib/gear.js` — logic อ่าน/เขียนแต่ละชีต
- `lib/writeLock.js` — คิวกันสองคนเขียนชีตเดียวกันพร้อมกันจนข้อมูลหาย (ใน instance เดียวกัน)
- `lib/auth.js`, `lib/rateLimit.js` — เช็คโหมดผู้บริหารและกันเดารหัสผ่าน
